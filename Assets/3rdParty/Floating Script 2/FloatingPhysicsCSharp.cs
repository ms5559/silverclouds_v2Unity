...
public class FloatingPhysicsCSharp : MonoBehaviour {
	...
	void FixedUpdate () {   //FixedUpdate is used because it is used with physics
		...
		if(timer < randomtime) { //increments timer
			timer += Time.deltaTime;
		}
		if(timer >= randomtime){    //This adds the torque that was executed at the start again every 10 seconds to have the object continue to turn slightly.
			timer = 0;
